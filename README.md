...
**Map.** The 39-state target board. States are filled by target type; a state carrying
...
| What | Where | Notes |
| --- | --- | --- |
| Target mixes, chapter status, districts, partners | `src/data/states.ts` | `STATE_SOURCE`, one entry per state |
