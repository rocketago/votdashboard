 ...
 *
 * This file is structured to match what `scripts/sync-airtable.mjs` would generate once a
 * "Story Bank" table is added to the VOT 2026 Soft Side Reports base. The shape is
 * intentional: a future sync script reads `Name`, `Quote`, `State`, `Districts` (linked),
 * and `Category` from Airtable and writes this file in the same format.
 *
 * Every story below is a fabricated placeholder. Names, quotes, and details were written
 * during the design session and have not been collected from real participants.
 * Replace verbatim once real stories are collected; the feed renders whatever is here.
 *
 * Re-run `npm run sync` (once the sync script covers this table) to refresh it.
 */
