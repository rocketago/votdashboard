...
import { NATIONAL, STORIES, buildScopeIndex, type ScopeIndex } from '../data/stories'
...
/** Every filter key the Story Bank sidebar offers, derived from the feed itself. */
...
export function useStoryFilters(): StoryFilters {
  const index = useMemo(() => buildScopeIndex(STORIES), [])
